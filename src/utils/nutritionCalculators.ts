...
export function calculateEntryNutrition(entry: FoodEntry): NutritionInfo {
  ...
  return {
    calories: entry.food.nutritionPer100g.calories * multiplier,
    protein: entry.food.nutritionPer100g.protein * multiplier,
    carbs: entry.food.nutritionPer100g.carbs * multiplier,
    fat: entry.food.nutritionPer100g.fat * multiplier,
    fiber: (entry.food.nutritionPer100g.fiber || 0) * multiplier,
    sugar: (entry.food.nutritionPer100g.sugar || 0) * multiplier,
    sodium: (entry.food.nutritionPer100g.sodium || 0) * multiplier,
  };
}
